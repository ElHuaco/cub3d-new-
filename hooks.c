...
/*   Updated: 2020/07/06 12:50:17 by aleon-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	camera_rotation(int key, t_vars *var)
{
	...
	{
		temp = var->dirX;
		var->dirX = var->dirX * cos(0.1) - var->dirY * sin(0.1);
		var->dirY = temp * sin(0.1) + var->dirY * cos(0.1);
		temp = var->planeX;
		var->planeX = var->planeX * cos (0.1) - var->planeY * sin(0.1);
		var->planeY = temp * sin(0.1) + var->planeY * cos(0.1);
	}
}
...
int			camera_update(int key, t_vars *var)
{
	...
	{
		var->posX += (((key == UP) || (key == W) || (key == D))
			- ((key == DOWN) || (key == S) || (key == A))) * var->dirX * 0.01;
		var->posY += (((key == UP) || (key == W) || (key == D))
			- ((key == DOWN) || (key == S) || (key == A))) * var->dirY * 0.01;
	}
	if (((var->map->val[(int)var->posY][(int)var->posX] == '0')
		|| var->map->val[(int)var->posY][(int)var->posX] == '2')
		&& (var->map->val[(int)(var->posY + 0.01)][(int)(var->posX + 0.01)] != '1')
		&& (var->map->val[(int)(var->posY - 0.01)][(int)(var->posX + 0.01)] != '1')
		&& (var->map->val[(int)(var->posY + 0.01)][(int)(var->posX - 0.01)] != '1')
		&& (var->map->val[(int)(var->posY - 0.01)][(int)(var->posX - 0.01)] != '1'))
		ray_caster(var);
